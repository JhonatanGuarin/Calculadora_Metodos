...
def convert_to_g_function(equation: str) -> str:
    """
    Convierte una ecuación f(x) = 0 a la forma x = g(x).

    Args:
        equation: La ecuación en formato string

    Returns:
        La función g(x) en formato string
    """
    try:
        x = sp.Symbol('x')

        # Usar el parser mejorado con transformaciones
        transformations = standard_transformations + (implicit_multiplication_application,)
        expr = parse_expr(equation, transformations=transformations, local_dict={'E': sp.E, 'exp': sp.exp})

        # Intentar despejar x
        g_expr = sp.solve(expr, x)

        if g_expr:
            # Si hay múltiples soluciones, devolver la primera
            return str(g_expr[0])
        else:
            raise ValueError("No se pudo despejar x de la ecuación. Intente proporcionar directamente la función g(x).")
    except Exception as e:
        raise ValueError(f"Error al convertir a función g(x): {str(e)}")