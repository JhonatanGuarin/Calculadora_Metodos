...
class EquationParser:
    def __init__(self, equation_str: str):
        """
        Inicializa el parser de ecuaciones para el método de Newton-Raphson.
        ...
        # Preprocesar y validar la ecuación
        self.processed_equation_str = self._preprocess_equation(equation_str)
        self._validate_equation_syntax(self.processed_equation_str)

        # Parsear la ecuación y su derivada
        self.equation = self._parse_symbolic_equation(self.processed_equation_str)
        self.derivative = sp.diff(self.equation, self.x)

        # Convertir a funciones numéricas
        self.func = self._create_safe_function(self.equation)
        self.derivative_func = self._create_safe_function(self.derivative)
    ...
    def _validate_equation_syntax(self, equation_str: str) -> None:
        """
        Valida la sintaxis de la ecuación.
        """
    ...
    def _parse_symbolic_equation(self, equation_str: str):
        """
        ...
        # Verificar si la función es diferenciable
        try:
            derivative = sp.diff(equation, self.x)
            # Probar si la derivada es evaluable
            test_value = 1.0
            derivative_value = derivative.subs(self.x, test_value)
            if derivative_value.has(sp.oo) or derivative_value.has(-sp.oo):
                raise ValueError("La derivada de la función contiene infinitos. La función puede no ser diferenciable en todos los puntos.")
        except Exception as e:
            if "cannot determine truth value" in str(e):
                # Este error puede ocurrir con expresiones complejas pero válidas
                pass
            else:
                raise ValueError(f"Error al calcular la derivada: {str(e)}. Asegúrese de que la función sea diferenciable.")

        return equation

    def _create_safe_function(self, sympy_expr):
        """
        Convierte una expresión simbólica en una función lambda segura.
        """
    ...
    def _test_function(self, func):
        """
        Prueba la función con un valor para verificar que es evaluable.
        """
        try:
    ...
    def evaluate_derivative(self, x_value: float) -> float:
        """Evalúa la derivada de la función en un punto dado."""
        return self.derivative_func(x_value)

    def get_function_expression(self) -> str:
        """Retorna la expresión de la función como string."""
        return str(self.equation)

    def get_derivative_expression(self) -> str:
        """Retorna la expresión de la derivada como string."""
